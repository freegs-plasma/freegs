...
def test_two_turns():
    ...
    assert np.isclose(circuit.controlBz(0.3, 0.1),
                      mcoil.controlBz(0.3, 0.1))
...
def test_move_R():
    ...
    dR = 0.6
    coil1 = MultiCoil([1.1, 0.2], [1.2, -0.3], current = 100., mirror=False)
    coil2 = MultiCoil([1.1 + dR, 0.2 + dR], [1.2, -0.3], current = 100., mirror=False)
...
def test_move_Z():
    ...
    dZ = 0.4
    coil1 = MultiCoil([1.1, 0.2], [1.2, -0.3], current = 100., mirror=False)
    coil2 = MultiCoil([1.1, 0.2], [1.2 + dZ, -0.3 + dZ], current = 100., mirror=False)
