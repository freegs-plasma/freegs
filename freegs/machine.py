...
class MultiCoil:
    ...
    # Definitions dependent on input parameters
    ...
    def getForces(self, equilibrium):
        ...
        Br = equilibrium.Br(self.coil_centre_R, self.coil_centre_Z)
        Bz = equilibrium.Bz(self.coil_centre_R, self.coil_centre_Z)
        self.current = current
        ...
        # In cgs units f = I^2/(c^2 * R) * (ln(8*R/a) - 1 + xi/2)
        # In SI units f = mu0 * I^2 / (4*pi*R) * (ln(8*R/a) - 1 + xi/2)
        self_fr = (mu0 * total_current**2 / (4.*np.pi*self.coil_centre_R)) * (np.log(8.*self.coil_centre_R/minor_radius) - 1 + self_inductance/2.)

        Ltor = 2*np.pi*self.coil_centre_R  # Length of coil
        return np.array([ (total_current * Bz  + self_fr) * Ltor, # Jphi x Bz = Fr, self force always outwards
                          -total_current * Br * Ltor]) # Jphi x Br = - Fz
