...
# ITER-like 15MA scenario for freeGS
# Coils positions from: A. Portone, "Perturbed solutions of fixed boundary MHD equilibria", 2004, Nuclear Fusion 44 265

def ITER():
    
    def PFcorners(r, z, dr, dz):
        return [
            [r - dr/2, z - dz/2],
            [r + dr/2, z - dz/2],
            [r + dr/2, z + dz/2],
    ...
    coils = [
                # CS
        ("CS3U", ShapedCoil(PFcorners(1.69,  5.00, 0.75, 2.00), control=True)),
        ("CS2U", ShapedCoil(PFcorners(1.69,  3.00, 0.75, 2.00), control=True)),
        ("CS1U", ShapedCoil(PFcorners(1.69,  1.00, 0.75, 2.00), control=True)),
        ("CS1L", ShapedCoil(PFcorners(1.69, -1.00, 0.75, 2.00), control=True)),
        ("CS2L", ShapedCoil(PFcorners(1.69, -3.00, 0.75, 2.00), control=True)),
        ("CS3L", ShapedCoil(PFcorners(1.69, -5.00, 0.75, 2.00), control=True)),
        
                # PF
        ("PF1", ShapedCoil(PFcorners( 3.95,  7.56, 0.97, 0.98), control=True)),
        ("PF2", ShapedCoil(PFcorners( 8.31,  6.53, 0.65, 0.60), control=True)),
        ("PF3", ShapedCoil(PFcorners(11.94,  3.27, 0.71, 1.13), control=True)),
        ("PF4", ShapedCoil(PFcorners(11.91, -2.24, 0.65, 1.13), control=True)),
        ("PF5", ShapedCoil(PFcorners( 8.40, -6.73, 0.82, 0.95), control=True)),
        ("PF6", ShapedCoil(PFcorners( 4.29, -7.56, 1.63, 0.98), control=True)),
    ]

    
    R = np.array([
    5.546610169491525, 5.271186440677965, 5.25, 5.165254237288135, 5.038135593220339, 4.889830508474576, 4.762711864406779, 4.656779661016949, 4.48728813559322,
    4.1906779661016955, 4.508474576271186, 4.529661016949152, 4.508474576271186, 4.466101694915254, 4.402542372881356, 4.338983050847457, 4.211864406779661, 4.127118644067797,
    4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304,
    4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304,
    4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.1059322033898304, 4.317796610169491, 4.932203389830509, 5.758474576271187,
    7.389830508474576, 7.898305084745763, 8.237288135593221, 8.385593220338983, 8.300847457627118, 7.919491525423729, 7.283898305084746, 6.033898305084746, 5.885593220338983,
    5.758474576271187,5.673728813559322, 5.610169491525424, 5.588983050847457, 5.567796610169491, 5.567796610169491, 5.546610169491525
    ])

    Z = np.array([
    -4.529661016949152, -4.275423728813561, -4.0, -3.872881355932204, -3.7669491525423737, -3.724576271186441, -3.724576271186441, -3.7457627118644083, -3.9152542372881367, -3.8940677966101696, -3.194915254237289,
    -3.046610169491526, -2.9194915254237284, -2.8135593220338997, -2.7288135593220346, -2.6440677966101696, -2.5805084745762716, -2.5169491525423737, -2.305084745762713, -2.15677966101695, -1.966101694915256, 
    -1.8177966101694931, -1.6059322033898322, -1.2669491525423746, -1.0550847457627128, -0.8008474576271194, -0.546610169491526,
    -0.33474576271186507, -0.1864406779661021, 0.02542372881355881, 0.2372881355932197, 0.49152542372881314, 0.766949152542372, 1.1059322033898296, 
    1.4025423728813555, 1.7838983050847457, 2.1440677966101696, 2.504237288135592, 2.8008474576271176, 3.0762711864406773, 3.372881355932203, 3.5847457627118637, 4.326271186440677, 4.7076271186440675, 4.538135593220338,
      3.1822033898305078, 2.46186440677966, 1.7203389830508469, 0.6398305084745752, -0.41949152542373014, -1.3093220338983054, -2.2627118644067803, -3.2584745762711886, -3.3220338983050848,
    -3.40677966101695, -3.5127118644067803, -3.5974576271186454, -3.703389830508476, -3.788135593220341, -4.148305084745765, -4.529661016949152 
    ])
    ...
    R0, a_minor, B0 = 6.2, 2.0, 5.3
    profiles = ConstrainPaxisIp(eq, paxis=8.0e5, Ip=15.0e6, fvac=R0*B0)

    x_lower = (5.12, -3.4)   
    x_upper = (5.0,  4.4)   
    xpoints = [x_lower, x_upper]
    isoflux = [
        x_lower + (R0 + a_minor, 0.4),   
        x_lower + (R0 - a_minor, 0.4),   
        x_lower + (R0,           3.7),   
        x_upper + (R0 + a_minor, 0.4),   
        x_upper + (R0 - a_minor, 0.4),
    ]

    constrain = control.constrain(xpoints=xpoints, isoflux=isoflux, gamma=1e-12)
