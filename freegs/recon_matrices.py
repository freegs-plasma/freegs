...
# Greens Matrix (nm x N)
def get_G(tokamak, eq):
    """
    ...
    #Creating Grid
    N = eq.nx * eq.ny
    G = np.zeros((len(tokamak.sensors), N))
    dR = eq.R[1, 0] - eq.R[0, 0]
    dZ = eq.Z[0, 1] - eq.Z[0, 0]

    for w, sensor in enumerate(tokamak.sensors):
        dim = eq.R.shape
        ...
        if isinstance(sensor, machine.FluxLoopSensor):
            for i in range(dim[0]):
                for j in range(dim[1]):
                    greens_matrix[i, j] = dR*dZ*Greens(eq.R[i,j],eq.Z[i,j],sensor.R,sensor.Z)
        ...
        if isinstance(sensor, machine.PoloidalFieldSensor):
            for i in range(dim[0]):
                for j in range(dim[1]):
                    greens_matrix[i, j] = dR*dZ*(GreensBr(eq.R[i,j],eq.Z[i,j],sensor.R,sensor.Z)*np.cos(sensor.theta) + GreensBz(eq.R[i,j],eq.Z[i,j],sensor.R,sensor.Z)*np.sin(sensor.theta))
...
# Coil Greens matrix (nm x ncoil)
def get_Gc(tokamak,eq):
    n_coils = len(tokamak.coils)
    Gc = np.zeros((len(tokamak.sensors), n_coils))
    dR = eq.R[1, 0] - eq.R[0, 0]
    dZ = eq.Z[0, 1] - eq.Z[0, 0]

    for w, sensor in enumerate(tokamak.sensors):
        for coil_num, coil in enumerate(tokamak.coils):
            ...
            if isinstance(sensor, machine.RogowskiSensor):
                ...
                # If coil is a circuit, find coils inside circuit
                    ...
                    for name, sub_coil, multiplier in coil.coils:
                        ...
                        if isinstance(sub_coil, machine.FilamentCoil):
            ...
            if isinstance(sensor, machine.FluxLoopSensor):
                Gc[w, coil_num] += coil.controlPsi(sensor.R, sensor.Z)

            if isinstance(sensor, machine.PoloidalFieldSensor):
                Gc[w, coil_num] += np.cos(sensor.theta)*coil.controlBr(sensor.R,sensor.Z) + np.sin(sensor.theta)*coil.controlBz(sensor.R,sensor.Z)

    return Gc


# Vessel Filament Greens Matrix (nm x nfils)
def get_Gfil(tokamak,eq):
    n_fils = 0
    ...
    Gfil = np.zeros((len(tokamak.sensors), n_fils))
    dR = eq.R[1, 0] - eq.R[0, 0]
    dZ = eq.Z[0, 1] - eq.Z[0, 0]
    ...
    for w, sensor in enumerate(tokamak.sensors):
        ...
        for fil in tokamak.vessel:
            if isinstance(fil, machine.Filament):
                ...
                if isinstance(sensor, machine.FluxLoopSensor):
                    Gfil[w, fil_num] = fil.controlPsi(sensor.R, sensor.Z)

                if isinstance(sensor, machine.PoloidalFieldSensor):
                    Gfil[w, fil_num] = np.cos(sensor.theta) * fil.controlBr(
            ...
            if isinstance(fil, machine.group_of_Filaments):
                for sub_fil in fil.filaments:
                    ...
                    if isinstance(sensor, machine.FluxLoopSensor):
                        Gfil[w, fil_num] += sub_fil.controlPsi(sensor.R, sensor.Z)

                    if isinstance(sensor, machine.PoloidalFieldSensor):
                        Gfil[w, fil_num] += np.cos(
                            sensor.theta) * sub_fil.controlBr(
                            sensor.R, sensor.Z) + np.sin(
                            sensor.theta) * sub_fil.controlBz(sensor.R, sensor.Z)
