...
def traceFieldLines(eq, solwidth=0.03, nlines=10, nturns=50, npoints=200, axis=None):
    ...
    psifunc = interpolate.RectBivariateSpline(eq.R[:,0], eq.Z[0,:], (psi - psi_axis) / (psi_bndry - psi_axis))

    rmid, zmid = critical.find_psisurface(eq, psifunc,
                                          r0, z0,
                                          r0 + 10., z0,
                                          psival=1.0)
    ...
    forward = LineCoordinates(line_forward[:,:,0],
                              line_forward[:,:,1],
                              line_forward[:,:,2])

    backward = LineCoordinates(line_backward[:,:,0],
                               line_backward[:,:,1],
                               line_backward[:,:,2])
