...
xx, yy = meshgrid(
    linspace(eq1.Rmin, eq1.Rmax, 65), linspace(eq1.Zmin, eq1.Zmax, 129), indexing="ij"
)
psi = exp(-((xx - 1.0) ** 2 + (yy) ** 2) / 0.3**2)
eq1._updatePlasmaPsi(psi)

xx, yy = meshgrid(
    linspace(eq2.Rmin, eq2.Rmax, 65), linspace(eq2.Zmin, eq2.Zmax, 65), indexing="ij"
)
psi = exp(-((xx - 1.0) ** 2 + (yy) ** 2) / 0.3**2)
...
profiles1 = freegs.jtor.ConstrainBetapIp(eq1, 0.1, 1e6, 1.0)
profiles2 = freegs.jtor.ConstrainBetapIp(eq2, 0.1, 1e6, 1.0)

jtor1 = profiles1.Jtor(eq1.R, eq1.Z, eq1.psi(), eq1.psi_bndry)
jtor2 = profiles2.Jtor(eq2.R, eq2.Z, eq2.psi(), eq2.psi_bndry)
